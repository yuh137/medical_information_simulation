...
export function getAllDataFromStore<T>(
  storeName: string,
...
export async function isStoreEmpty(storeName: string): Promise<boolean> {
  const data = await getAllDataFromStore(storeName);
  if (Array.isArray(data) && data.length === 0) {
    return true; // Store is empty
  } else if (Array.isArray(data)) {
    return false; // Store is not empty
  } else {
    console.error("Error retrieving data:", data);
    return false; // Handle as non-empty in case of an error
  }
}

export async function getAccountData(): Promise<Admin[] | Student[] | null> {
  const isStudentStoreEmpty = await isStoreEmpty("students");

  if (!isStudentStoreEmpty) {
    const students = await getAllDataFromStore<Student>("students");
    if (Array.isArray(students)) {
      return students;
    } else {
      console.error("Error fetching students data:", students);
      return null; // Handle error case
    }
  } else {
    const admins = await getAllDataFromStore<Admin>("admins");
    if (Array.isArray(admins)) {
      return admins;
    } else {
      console.error("Error fetching admins data:", admins);
      return null; // Handle error case
    }
  }
}
...
export function getAllDataByFileName(
  storeName: string,
  fileName: string,
): Promise<QCTemplateBatch[]> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open("MIS_database");
    request.onsuccess = () => {
      const db = request.result;
...
export function saveToDB<
  T extends { fileName: string; lotNumber: string; closedDate: string },
>(storeName: string, data: T): Promise<void> {
  return new Promise((resolve, reject) => {
    let request = indexedDB.open("MIS_database");
    request.onsuccess = () => {
      const db = request.result;
