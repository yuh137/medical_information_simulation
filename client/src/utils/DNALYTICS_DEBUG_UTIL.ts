...
export async function DEBUG_add_molecular_data_to_idb(QCPanels: string[]): void {
	try {
		await DEBUG_ADD(QCPanels);
...
function DEBUG_ADD(QCPanels: string[]): Promise<void> {
	return new Promise((resolve, reject) => {
		...
		request.onsuccess = (event) => {
			...
			const addRequest = objectStore.add(QCPanels.map((item) => { fileName: item });
