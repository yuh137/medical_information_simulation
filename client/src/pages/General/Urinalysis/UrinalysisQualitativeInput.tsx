...
import { Dip, Osmolality, DrugScreen, Clinitest, Acetest, Ictotest} from "../../../utils/UA_MOCK_DATA";
...
interface QCRangeElements {
  ...
  selectedExpectedRange?: string[]; // Add this property
}

export const UrinalysisQualitativeInput = (props: { name: string }) => {
  ...
  const initialData =
  item?.includes('dip') ? Dip :
  item?.includes('osmolality') ? Osmolality :
  item?.includes('drug') ? DrugScreen :
  item?.includes('clinitest') ? Clinitest :
  item?.includes('acetest') ? Acetest :
  item?.includes('ictotest') ? Ictotest:
  Dip;
  ...
  const saveQC: SubmitHandler<UABFQCTemplateBatch> = async (data) => {
    const qcDataToSave: UABFQCTemplateBatch = {
      ...
      analytes: QCElements.map(({ analyteName, analyteAcronym, unit_of_measure, electrolyte, mean, std_devi, min_level, max_level, expected_range, selectedExpectedRange }) => ({
        analyteName, analyteAcronym, unit_of_measure, electrolyte, mean,
        std_devi, min_level, max_level,expected_range, selectedExpectedRange
      })),
    };
  ...
  return (
    <>
      <NavBar name={`Urinalysis QC Builder`} />
      ...
      <div className="basic-container relative sm:space-y-2 pb-18">
        ...
        <div className="mb-4">
          ...
          <div className="flex justify-center space-x-4 mt-2">
            ...
            <Link to={`/UA_fluids/edit_qc/levyjennings/${item}`}>
            ...
            <Link to={`/UA_fluids/edit_qc/${item}`}>
              <ButtonBase className="sm:w-48 !text-lg !border !border-solid !border-[#6A89A0] !rounded-lg sm:h-16 !bg-[#C5E0B4] transition ease-in-out duration-75 hover:!bg-[#00B050] hover:!border-4 hover:!border-[#385723] hover:font-semibold">
              Qualitative
              </ButtonBase>
          </Link>
