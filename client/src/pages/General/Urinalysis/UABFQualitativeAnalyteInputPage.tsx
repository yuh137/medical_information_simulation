...
// }
const QualitativeAnalyteInputPage = (props: { name: string }) => {
  const { theme } = useTheme();
  const inputRefs = useRef<HTMLInputElement[]>([]);
  const analyteNameRefs = useRef<HTMLDivElement[]>([]);
  const { username } = useAuth();
  const [qcData, setQcData] = useState<UABFQCTemplateBatch | null>(null);
  ...
  // get the stored selected expected ranges to check against the user entered range
  // const [selectedExpectedRanges, setSelectedExpectedRanges] = useState<string[][]>([]);
  // useEffect(() => {
  //   const storedRanges = localStorage.getItem('selectedExpectedRanges');
  //   if (storedRanges) {
  //     try {
  //       const parsedRanges = JSON.parse(storedRanges);
  //       if (Array.isArray(parsedRanges) && parsedRanges.every(analyte => Array.isArray(analyte.expected_range))) {
  //         setSelectedExpectedRanges(parsedRanges);
  //       } else {
  //         console.error('Unexpected data structure:', parsedRanges);
  //       }
  //     } catch (error) {
  //       console.error('Error parsing stored ranges:', error);
  //     }
  //   }
  // }, []);
  ...
  const handleInputChange = (index: number, value: string, selectedExpectedRange: string[]) => {
    const newValues = [...analyteValues];
    newValues[index] = value;
    setAnalyteValues(newValues);
    if (!selectedExpectedRange.includes(value)) {
      if (!invalidIndexes) {
  ...
  const handleKeyPress = (event: React.KeyboardEvent, index: number) => {
    if (event.key === "Enter" && index < inputRefs.current.length - 1 && inputRefs.current[index + 1]) {
  ...
  const reportPDF = (analyteValues?: string[], QCData?: UABFQCTemplateBatch) => {
    ...
    return (
      <Document style={tw("border border-solid border-black")}>
        <Page style={tw("py-8 px-16 border border-solid border-black")}>
          ...
          <Text style={tw("text-[22px] mb-8 text-center")}>{QCData?.fileName} QC</Text>
          <View style={tw("flex-row justify-between mb-2")}>
              <Text style={tw("font-[700] text-[15px] flex-1")}>Analytes</Text>
              <Text style={tw("font-[700] text-[15px] flex-1 text-center")}>Value</Text>
              <Text style={tw("font-[700] text-[15px] flex-1 text-right")}>Range</Text>
            </View>
            <View style={tw("w-full h-[1px] bg-black mb-4")} />

            <View>
              {analyteValues?.map((value, index) => (
                <View style={tw("flex-row justify-between mb-2")} key={index}>
                  {/* Analytes */}
                  <Text
                    style={tw(
                      `flex-1 text-[13px] ${
                        invalidIndexArray?.includes(index) ? "text-red-500" : "text-green-500"
                      }`
                    )}
                  >
                  ...
                  <Text
                    style={tw(
                      `flex-1 text-[13px] text-right ${
                        invalidIndexArray?.includes(index) ? "text-red-500" : "text-green-500"
                      }`
                    )}
                  >
                    {QCData?.analytes[index].selectedExpectedRange?.join(", ") || "N/A"}
                  </Text>
                </View>
              ))}
            </View>
          ...
          <Text style={tw("mt-8 text-[13px]")}>Approved by: {username || "unknown"}</Text>
  ...
  const isValid = useMemo(() => {
    if (!qcData) return false;
    return analyteValues.length === qcData.analytes.length &&
      analyteValues.every((value, index) => {
        const range = qcData.analytes[index].selectedExpectedRange;
        return range ? range.includes(value) : false;  // Ensure expected_range is defined before using .includes
      });
  }, [analyteValues, qcData]);
          ...
          {qcData.analytes.map((item, index) => (
            ...
            >
              <QualitativeAnalyte
                ...
                expected_range={item.expected_range || []}  // Handle possible undefined
                measUnit={item.unit_of_measure}
                selectedExpectedRange={item.selectedExpectedRange || []}
                handleInputChange={(val: string) => {
                  handleInputChange(index, val, item.selectedExpectedRange || []);
                }}
                ref={(childRef: {
                  ...
                  if (childRef) {
                    inputRefs.current.push(
                      childRef.inputRef.current as HTMLInputElement
                    );
                    analyteNameRefs.current.push(
                      childRef.nameRef.current as HTMLDivElement
                    );
                  }
                }}
              />
...
{/*       
    <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)}>
      <div className={`modal-container absolute top-1/1 left-1/2 sm:w-[50svw] sm:h-[80svh] bg-[${theme.secondaryColor}] border-2 border-solid border-[#6781AF] rounded-xl sm:-translate-x-1/2 sm:translate-y-12 flex flex-col items-center sm:py-6 relative overflow-scroll sm:space-y-4`}>
        ...
        <div className="invalid-items sm:w-[80%]">
          ...
          {invalidIndexArray && invalidIndexArray.length > 0 && (
            <div className="invalid-items-comments flex flex-col sm:space-y-6">
              {invalidIndexArray.map((invalidItem, index) => (
                <div className="comment flex sm:space-x-12 h-fit" key={invalidItem}>
                  ...
                  <div className="expected-ranges">
                    <h3>Expected Ranges for {analyteNameRefs.current[invalidItem]?.innerText}:</h3>
                    <ul>
                      {selectedExpectedRanges[invalidItem]?.expected_range?.map((range, rangeIndex) => (
                        <li key={rangeIndex}>{range}</li>
                      ))}
                    </ul>
                  </div>
                </div>
              ...
              <ButtonBase
                className={`!rounded-lg sm!my-10 sm:!py-6 sm:!px-10 !bg-[${theme.secondaryColor}] !border-[1px] !border-solid !border-[${theme.primaryBorderColor}] transition ease-in-out hover:!bg-[${theme.primaryHoverColor}] hover:!border-[#2F528F] sm:w-1/2 self-center`}
                onClick={() => {
              ...
              {invalidIndexArray.map((invalidItem, index) => (
                <div className="comment flex sm:space-x-12 h-fit" key={invalidItem}>
                  ...
                  {/* <div className="expected-ranges">
                    <h3>Expected Ranges for {analyteNameRefs.current[invalidItem]?.innerText}:</h3>
                    <ul>
                      {selectedExpectedRanges[invalidItem]?.map((range, rangeIndex) => (
                        <li key={rangeIndex}>{range}</li>
                      ))}
                    </ul>
                  </div> */}
...
export default QualitativeAnalyteInputPage;
