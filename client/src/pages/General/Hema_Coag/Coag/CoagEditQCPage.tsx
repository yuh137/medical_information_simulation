...
import { Department, qcTypeLinkList, CoagTypeLinkList } from "../../../../utils/utils";
        ...
        <div className="button-container sticky -bottom-12 flex justify-center sm:-translate-y-12 sm:space-x-36 z-2 bg-white sm:py-6">
          <ButtonBase
            ...
            onClick={() => {
              if (selectedItem) {
                navigate({
                  pathname: `/hema_coag/coag/coag_editQC/${selectedItem}`,
                  search: createSearchParams({
                    dep: Department.Hematology.toString(),
                    name: CoagTypeLinkList.find(item => item.link == selectedItem)?.name ?? ""
                  }).toString()
                })
              } else {
          ...
          <ButtonBase className="sm:w-48 !text-lg !border !border-solid !border-[#6A89A0] !rounded-lg sm:h-16 !bg-[#C5E0B4] transition ease-in-out duration-75 hover:!bg-[#00B050] hover:!border-4 hover:!border-[#385723] hover:font-semibold"
            onClick={async () => {
              if (selectedItem) {
                ...
                const name = CoagTypeLinkList.find(item => item.link == selectedItem)?.name ?? "";
                try {
...
export default CoagulationEditQC;
