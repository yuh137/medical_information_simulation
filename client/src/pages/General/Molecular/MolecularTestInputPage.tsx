	...
	const loadQCData = async () => {
		...
		const initialRanges = panelAnalytes.reduce((acc, item) => {
			if (item.type === 'QualitativeAnalyte') {
				acc[item.analyteName] = item.expectedRange as string;
			}
			else {
	...
	const handleSubmit = async () => {
		...
		for (let i = 0; i < qcPanelRef.current.analytes.length; i++) {
			...
			if (analyte.type === 'QualitativeAnalyte') {
				analyte.expectedRange = ranges[analyte.analyteName];
			}
		}
		const requestBody = {
			...
			"analytes": qcPanelRef.current.analytes.map(analyte => ({
				...
				"minLevel": analyte.minLevel ? parseFloat(parseFloat(analyte.minLevel.toString()).toFixed(2)) : null,
				"maxLevel": analyte.maxLevel ? parseFloat(parseFloat(analyte.maxLevel.toString()).toFixed(2)) : null,
		...
		const token = localStorage.getItem("token");
		const authToken: AuthToken = JSON.parse(token as string);
		const res = await fetch(`${process.env.REACT_APP_API_URL}/AdminQCLots/UpdateQCLot/${qcPanelRef.current.adminQCLotID}`, {
			method: "PUT",
			headers: {
				"Content-Type": "application/json",
				Accept: "application/json",
				...
				<div className="flex flex-row space-x-4">
					...
					<div className="opendate-input flex flex-col items-center bg-[#3A6CC6] rounded-xl sm:space-y-2 sm:px-2">
						...
						<DatePicker
							...
							value={openDateInput ? dayjs(openDateInput) : dayjs()}
							format="MM/DD/YYYY"
							onChange={(value) => {
								value && setOpenDateInput(value.toISOString());
							}
							}
						/>
					</div>
					<div className="closeddate-input flex flex-col items-center bg-[#3A6CC6] rounded-xl sm:space-y-2 sm:px-2">
						...
						<DatePicker
							...
							value={closedDateInput ? dayjs(closedDateInput) : dayjs()}
							format="MM/DD/YYYY"
							onChange={(value) => {
								value && setClosedDateInput(value.toISOString());
							}
							}
						/>
					</div>
