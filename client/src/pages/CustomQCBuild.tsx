...
const CustomQCBuild = (props: { name: string; link: string }) => {
  ...
  useEffect(() => {
    setQCElements(new Array(numRows).fill({}).map((_, i) => ({ analyteName: '', analyteAcronym: '', unit_of_measure: '', min_level: '', max_level: '', mean: '', std_devi: '', electrolyte: false })));
  }, [numRows]);
  ...
  return (
    ...
    <>
      ...
      <div className="relative mt-8 sm:w-[94svw] sm:mx-auto bg-[#CFD5EA]"> {/* This container should be relative */}

      {showControls && (

<div className="absolute top-0 left-0 right-0 flex flex-col items-center justify-center" style={{ zIndex: 2 }}>
<ButtonBase  
    onClick={toggleDropdown} 
    className="!rounded-lg sm:w-80 sm:w-48 sm:h-36 sm:!my-12 !bg-[#C5E0B4] !border-[1px] !border-solid !border-[#6A89A0] transition ease-in-out hover:!bg-[#00B050] hover:!border-[#385723] hover:!border-[4px] !px-1"
  ...
  {dropdownVisible && (
    ...
    >
      ...
      {Array.from({ length: 10 }, (_, i) => i + 1).map(number => (
        <option key={number} value={number} className="text-center">{number}</option>
      ))}
    </select>
  )}
</div>
